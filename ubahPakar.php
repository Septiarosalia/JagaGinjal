    ...
    <div class="kanan">
        <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-between ml-4 py-5">
                <h1 class="h3 mb-0 text-gray-800">Ubah Pakar</h1>
            </div>

            <!-- Form Container -->
            <div class="form-container">
                <form action="function.php?act=ubahPakar&id_user=<?= $user['id_user']; ?>" method="POST">
                    <div class="form-group">
                        <h3>Form Ubah Pakar</h3>
                    </div>
                    <div class="form-group">
                        <label for="nama">Nama Pakar</label>
                        <input type="text" class="form-control" id="nama" name="nama" 
                               value="<?= htmlspecialchars($user['nama']); ?>" placeholder="Masukkan Nama">
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" class="form-control" id="email" name="email" 
                               value="<?= htmlspecialchars($user['email']); ?>" placeholder="Masukkan Email">
                    </div>
                    <div class="form-group">
                        <label for="alamat">Alamat</label>
                        <input type="text" class="form-control" id="alamat" name="alamat" 
                               value="<?= htmlspecialchars($user['alamat']); ?>" placeholder="Masukkan Alamat">
                    </div>
                    <div class="form-group">
                        <label for="tgl_lahir">Tanggal Lahir</label>
                        <input type="date" class="form-control" id="tgl_lahir" name="tgl_lahir" 
                               value="<?= htmlspecialchars($user['tgl_lahir']); ?>">
                    </div>
                    <div class="form-group">
